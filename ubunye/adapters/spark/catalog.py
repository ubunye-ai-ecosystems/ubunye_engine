"""Set Unity Catalog and schema on a Spark backend.

Moved out of ``ubunye.core`` under issue #37: ``USE CATALOG`` / ``USE SCHEMA``
are Spark (and Unity) statements, so they are the Spark adapter's job, not the
engine core's. The behaviour is unchanged; only the home is.
"""
