...
3. Other frameworks in the client ecosystem (like Rialto from CPDS) solve this by exposing a Python API that runs natively inside Databricks.
...
Following the client CPDS pattern for Databricks Asset Bundles:
...
- What Activate is (client digital insurance, telematics, up to 40% cashback for good driving)
...
- Trip scoring features: hard_accel, hard_brake, legal, overall_client_v1, distance, trip_duration, late_drive_duration
...
- Rialto (CPDS) follows this same pattern: runner script + asset bundle + GitHub Actions deploy. Ubunye should follow the same pattern for consistency within client.
...
4. The asset bundle must follow client CPDS conventions
