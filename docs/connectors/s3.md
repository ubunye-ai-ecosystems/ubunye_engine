...
```yaml
    daily_sales:
      ...
      file_format: delta          # required: see the warning below
      mode: overwrite_partitions
      partitionBy: [dt]

    # Predicate form — replaces exactly what the expression matches
    daily_sales_scoped:
      format: s3
      path: s3://my-bucket/delta/sales/
      file_format: delta
...
!!! warning "Requires `file_format: delta` on a path"

    Spark honours `partitionOverwriteMode=DYNAMIC` only for `INSERT OVERWRITE`
    into a table; on a path write it is ignored and the whole dataset is
    replaced (verified against Spark 4.1). `mode: overwrite_partitions` on a
    Parquet path therefore raises `SinkWriteError` instead of destroying the
    other partitions. Delta honours it, so set `file_format: delta` — or write
    to a table with the `hive` / `unity` connector.
...
| `mode` | see [Write modes](../config/io.md#write-modes) | No | Default `append`. All six modes supported; `merge` and `overwrite_partitions` require `file_format: delta` |
