"""Spark unit tests for the multi-task Titanic pipeline.
...
# Ensure both task dirs are importable
for d in (PIPELINE_DIR / "clean_data", PIPELINE_DIR / "aggregate"):
...
class TestCleanData:
    def test_adds_survived_label(self, raw_passengers):
        from transformations import clean_titanic  # from clean_data/

        result = clean_titanic(raw_passengers)
        labels = {r["PassengerId"]: r["survived_label"] for r in result.collect()}
        assert labels[1] == "yes"
        assert labels[2] == "no"

    def test_adds_age_group(self, raw_passengers):
        from transformations import clean_titanic
...
class TestAggregate:
    ...
    def cleaned_df(self, raw_passengers):
        ...
        return clean_titanic(raw_passengers)

    def test_groups_by_class_and_age(self, cleaned_df):
        # Need to reimport from aggregate's transformations
        sys.path.insert(0, str(PIPELINE_DIR / "aggregate"))
        # Force reload since clean_data's transformations is cached
        if "transformations" in sys.modules:
            del sys.modules["transformations"]
        from transformations import aggregate_survival

        result = aggregate_survival(cleaned_df)
