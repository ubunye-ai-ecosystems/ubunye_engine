"""Policy-device-exposure mapping transformation.

Ports the ``policy_device_mapping_etl`` logic into a Ubunye ``Task``:
...
3. Keep the latest policy version per ``(policy_number, lob_asset_id,
   effective_year_month)`` and the latest device record per
...
5. Right-join the corrected device frame onto the latest MI frame on
   ``device.item_no == mi.lob_asset_id`` and emit the final select.
...
def _latest_device_record(df: "Any") -> "Any":
    from pyspark.sql import functions as F
    from pyspark.sql.window import Window
...
def _correct_installation(device_latest: "Any", imei_first: "Any") -> "Any":
    from pyspark.sql import functions as F

    te = device_latest.alias("te")
...
def build_policy_device_mapping(
    ...
    device_latest = _latest_device_record(policy_device_details)
    imei_first = _imei_first_detection(user_imei)
    device_corrected = _correct_installation(device_latest, imei_first)

    joined = device_corrected.alias("device").join(
        mi_latest.alias("mi"),
        on=F.col("device.item_no") == F.col("mi.lob_asset_id"),
        how="right",
    )
    ...
    return joined.select(
        ...
        F.col("device.imei_first_detection_datetime"),
        F.col("device.installation_datetime_final").alias("installation_datetime"),
...
class PolicyDeviceMapping(Task):
    """Ubunye Task: policy/device/exposure mapping."""
