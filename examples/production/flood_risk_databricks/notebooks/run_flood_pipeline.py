...
# MAGIC %md
# MAGIC # Flood Risk - two-task pipeline (Databricks, Unity Catalog)
# MAGIC
...
# MAGIC `secret_scope` widget (default `flood-risk`). Unity Catalog
...
dbutils.widgets.text("telm_catalog", "", "Unity Catalog catalog (device mapping)")
dbutils.widgets.text("telm_schema", "", "Unity Catalog schema (device mapping)")
dbutils.widgets.text("address_source_table", "", "Unqualified source table name (id, address)")
dbutils.widgets.text("secret_scope", "flood-risk", "Databricks secret scope for API credentials")
