# Flood Risk — TomTom geocoding + JBA flood metrics (two-task pipeline)
...
**Databricks secret scope** (default name `flood-risk`, overridable):

```bash
databricks secrets create-scope flood-risk
databricks secrets put-secret flood-risk tomtom_api_key       # <TomTom API key>
databricks secrets put-secret flood-risk jba_basic_auth       # "Basic <base64 of user:pass>"
```
...
pytest examples/production/flood_risk_databricks/tests -v
```
...
```bash
cd examples/production/flood_risk_databricks
...
databricks bundle run flood_risk --target nonprod
```

The workflow in `.github/workflows/flood_risk_databricks.yml`
