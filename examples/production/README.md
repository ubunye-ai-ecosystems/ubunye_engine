...
| [`client_telematics_etl_databricks/`](./client_telematics_etl_databricks/) | Databricks serverless + UC (paid workspace only) | 3x UC tables (policy-device details, user-IMEI, MI exposure) | UC Delta table (policy/device/exposure mapping) |
| [`client_flood_risk_databricks/`](./client_flood_risk_databricks/) | Databricks serverless + UC (paid) | UC table with `(id, address)` | Unity Catalog Delta tables (`address_geocoded` → `address_flood_risk`) |
