...
def _broken_cfg(input_path: str) -> dict:
    ...
    return {
        ...
        "CONFIG": {
            ...
            "transform": {"type": "noop"},
            "outputs": {
                "source": {"format": "s3", "path": "/tmp/never_written", "mode": "overwrite"}
            },
        },
    }
...
def _run(
    cfg: dict, spark_session: SparkSession, lineage_base: str, fail: bool = False
) -> tuple[str, FileSystemLineageStore]:
...
class TestLineageIntegration:
    def test_successful_run_produces_lineage_file(self, spark, tmp_path):
        ...
        run_id, store = _run(
            _good_cfg(csv, str(tmp_path / "out")), spark, str(tmp_path / "lineage")
        )
